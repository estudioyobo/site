...
import './select.css'
import { Label, Group } from './styles'

const Select = ({ children, name }) => {
  const ref = useRef(null)

  useEffect(() => {
    if (ref.current) new SelectFx(ref.current)
  }, [ref])

  return (
    <Group>
      <Label htmlFor={name}>{name}:</Label>
      <select className='cs-select cs-skin-slide' ref={ref} name={name} id={name}>
        {children}
      </select>
    </Group>
  )
...
class SelectFx {
  ...
  _createSelectEl () {
    ...
    var optsEl = '<div class="cs-options"><ul>' + options + '</ul></div>'
    this.selEl = document.createElement('div')
    this.selEl.className = this.el.className
    this.selEl.tabIndex = this.el.tabIndex
    this.selEl.innerHTML =
      '<span class="cs-placeholder">' + this.selectedOpt.textContent + '</span>' + optsEl
